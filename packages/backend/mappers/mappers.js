...
class ConsultaPedidoMapper{
    pedidosSegun(user){
        switch (user.tipo){
            case TipoUsuario.VENDEDOR: return {vendedor: user}
            case TipoUsuario.COMPRADOR: return {comprador: user}
            default: return {}
        }
    }
}
...
class ProductosQPMapper{
    ...
    sortParamMapper(muestreo){
        if (muestreo.orderBy) {
            const campo = muestreo.orderBy === "precio" ? "precio" : "cantidadesVendidas";
            return {
                [campo]: muestreo.order === "desc" ? -1 : 1,
                _id: 1
            };
        }
        return { _id: 1 };
    ...
    /**
     * Construye un objeto de filtros para consultas en base a filtros crudos y un vendedor (si aplica).
     *
     * - Si no se quiere filtrar por vendedor, se puede pasar null.
     * - Si existe `busqueda`, se busca ese término en título, nombre y categorías.
     * - Si existen `categorias`, se filtra por coincidencia exacta en alguna categoría del producto.
     * - También se puede filtrar por un rango de precios (`minPrice`, `maxPrice`).
     *
     * @param {Object} filtrosCrudos - Filtros recibidos desde query params (ya parseados/validados con Zod).
     * @param {Usuario|null} vendedor - Vendedor al que pertenecen los productos (o `null` si no aplica).
     * @returns {Object} Objeto de filtros compatible con consultas de Mongoose.
     *
     * @example
     * const filtros = filterMapper(
     *   { busqueda: "verde", categorias: ["tés", "hierbas"], minPrice: 100 },
     *   { id: "123", nombre: "VendedorA" }
     * );
     * // filtros = {
     * //   vendedor: "123",
     * //   $or: [
     * //     { titulo: /verde/i },
     * //     { descripcion: /verde/i },
     * //     { "categorias.nombre": /verde/i }
     * //   ],
     * //   "categorias.nombre": { $in: ["tés", "hierbas"] },
     * //   precio: { $gte: 100 }
     * // }
     */
    filterMapper(filtrosCrudos, vendedor) {
        const filtros = vendedor ? { vendedor: vendedor.id } : {};

        if (filtrosCrudos.busqueda) {
            const regex = new RegExp(filtrosCrudos.busqueda.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i");
            filtros.$or = [
                { titulo: regex },
                { descripcion: regex },
        ...
        if (filtrosCrudos.isActive !== undefined) {
            filtros.activo = filtrosCrudos.isActive;
        }
...
export const consultaPedidoMapper = new ConsultaPedidoMapper()
